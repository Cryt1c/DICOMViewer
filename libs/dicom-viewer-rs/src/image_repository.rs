...
use dicom_pixeldata::{
    ...
    ndarray::{self, s, stack, Array3, ArrayBase, Axis, Dim, OwnedRepr},
...
impl ImageRepository {
    ...
    pub fn create_slice_array(&mut self) {
        ...
        self.slice_array = slice_array_255;
    ...
    pub fn get_image_from_array(&mut self, index: usize) -> Option<&Image> {
        let width = self.slice_array.dim().2 as u32;
        let height = self.slice_array.dim().1 as u32;
        let slice = self.slice_array.slice(s![index, .., ..,]);
